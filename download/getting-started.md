...
docpath: http://www.scala-lang.org/api/current/index.html
...
We assume that both the [Scala software](node/211) and the user environment are set up correctly. For example:
...
The `scalac` command compiles one (or more) Scala source file(s) and generates Java bytecode which can be executed on any [standard JVM](http://java.sun.com/docs/books/jvms/). The Scala compiler works similarly to [`javac`](http://java.sun.com/j2se/1.5.0/docs/tooldocs/solaris/javac.html), the Java compiler of the [Java SDK](http://java.sun.com/javase/).
