...
### A cross-format documentation engine for Scala
...
aims to simplify writing documentation for Scala library authors, so that they
can focus on the inherent difficulty of writing good docs.

The scope of the project is to continue the development of an existing
prototype. The project can start small with simple changes to get familiar with
the codebase, and gradually step up as progress happens.

Your end goal will be to implement the following features:
...
1. A series of modifications to the default website template that:
  ...
  1. Implement good (static) search and discovery capabilities.
  1. Implement an snappy local edit and preview experience.
